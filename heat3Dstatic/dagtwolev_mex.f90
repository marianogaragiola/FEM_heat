...
!
    ...
    !
    REAL(kind(0.0d0)) :: kapma
    kapma=MAX(kaptg_blocdia,kaptg_dampJac)
    IF (l .EQ. 1) THEN
       IF (wfo) THEN
    ...
    !
       IF (wfo) THEN
          IF (zerors) THEN
             ...
             call mexPrintf(lout//char(10))
          END IF
          WRITE(lout,910) IRANK,(i,i=0,ceiling(imult*kapma-repsmach)+1)
          call mexPrintf(lout//char(10))
...
910       FORMAT(i3,'* DomD  NPC /',20(i2,2x))
    ...
    IF (smoothtype.EQ.0 .AND. l.EQ.1 .AND. (.NOT.spd)) THEN
        ALLOCATE(w(2*n))
        CALL dag2l_omegaest(n,a,ja,ia,dt(l)%idiag,si1,w )
        DEALLOCATE(w)
    END IF
    !
...
!-----------------------------------------------------------------------
  SUBROUTINE dag2l_omegaest(n,a,ja,ia,idiag,si,w,iext)
    !
    USE dag2l_mem
    IMPLICIT NONE
...
!
    ...
    ijs=1
    npc=0
    nb_trs=ceiling(kaptg-repsmach)
    ALLOCATE(save_trs(nb_trs+1))
    save_trs=0
      nt=n
!
...
25     CONTINUE
       ...
       ELSE
          ...
          ind(ipair) = nc
          nmark = nmark+1
          itrs=max(1,ceiling(val))
          save_trs(itrs)=save_trs(itrs)+1
       END IF
      ENDDO
    IF (wfo) THEN
       i1=0
       save_trs(nb_trs)=save_trs(nb_trs)+save_trs(nb_trs+1)
       save_trs(nb_trs+1)=0
       DO i2=1,nb_trs
          i1=i1+save_trs(i2)
       END DO
